...
export const CloudDisk: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation }): JSX.Element => {
    ...
    const onRefresh = async () => {
      console.log('设备高度', Dimensions.get('window'), Dimensions.get('screen'), StatusBar.currentHeight);
          ...
          <Portal>
            <Animated.View style={[tw.absolute, { height: checkPublic ? 100 : 0, opacity: fadeTypeAnim.current, top: isIos ? 100 : 50 }]}>
              <View style={[tw.flex1, tw.justifyBetween, checkPublic ? tw.p2 : tw.p0, { borderRadius: 3, backgroundColor: colors.surface }]}>
                ...
                >
                  <Text style={[{ fontSize: 12, color: !cloudSeaDiskStore.isPublic ? colors.accent : colors.text, fontWeight: 'bold' }]}>私有资源</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    tw.flex1,
                ...
                >
                  <Text style={[{ fontSize: 12, color: cloudSeaDiskStore.isPublic ? colors.accent : colors.text, fontWeight: 'bold' }]}>共享资源</Text>
                </TouchableOpacity>
              </View>
            </Animated.View>
          </Portal>
              ...
              <View style={[tw.flex1, tw.justifyBetween, orderPublic ? tw.p2 : tw.p0, { borderRadius: 3, backgroundColor: colors.surface }]}>
                {cloudSeaDiskStore.orderList.slice().map((item, index) => {
                  return (
                    ...
                    >
                      <Text style={[{ fontSize: 12, color: cloudSeaDiskStore.isOrder === index ? colors.accent : colors.text, fontWeight: 'bold' }]}>
                        {item.title}
                      </Text>
                    </TouchableOpacity>
                  );
