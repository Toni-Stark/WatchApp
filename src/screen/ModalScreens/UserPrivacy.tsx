...
export const UserPrivacy: React.FunctionComponent<Props> = observer((props) => {
  ...
  const navi = (num: number) => {
    ...
    switch (num) {
      case 1:
        let urlOne = 'http://www.icst-edu.com/?xueyue/37.html';
        Linking.openURL(urlOne).then((res) => {
          console.log(res);
        });
        break;
      case 2:
        let urlTwo = 'http://www.icst-edu.com/?xueyue/32.html';
    ...
    <BaseView useSafeArea={false} ref={loading}>
      ...
      <View style={[tw.flex1, tw.p5]}>
        <Text style={[{ fontSize: 16 }]}>
          感谢您下载并使用智能手表APP！我们非常重视您的个人信息和隐私保护。为了更好地保障您的权益，请您认真阅读下面协议的全部内容，同意并接受全部条款后，开始使用我们的产品和服务。
        </Text>
