...
import { Animated, FlatList, Linking, Platform, RefreshControl, TextInput, TouchableHighlight, TouchableOpacity, View } from 'react-native';
import Icon from 'react-native-vector-icons/MaterialIcons';
import { useStore } from '../../../store';
import { t, throttle } from '../../../common/tools';
import { DiskFolder } from './DiskFolder';
import BottomSheet from 'reanimated-bottom-sheet';
import { AllResourcesAllListType } from './CloudSeaDiskStore';
...
export const CloudSeaDisk: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation }): JSX.Element => {
    ...
    const { cloudSeaDiskStore, uploadFilesStore, userStore, createVodStore } = useStore();
    ...
    let timer: any;
    const delayedFun = (callback: () => void) => {
      clearTimeout(timer);
      timer = setTimeout(() => {
        callback();
      }, 2500);
    ...
    useEffect(() => {
      return () => {
        cloudSeaDiskStore.screenName = undefined;
      };
    }, [cloudSeaDiskStore]);
    ...
    const fadeTypeIn = () => {
      ...
      Animated.timing(fadeTypeAnim.current, { toValue: 1, duration: 500, useNativeDriver: true }).start();
    };

    const fadeTypeOut = () => {
      Animated.timing(fadeTypeAnim.current, { toValue: 0, duration: 500, useNativeDriver: true }).start();
    };
    ...
    const fadeOrderIn = () => {
      ...
      Animated.timing(fadeOrderAnim.current, { toValue: 1, duration: 500, useNativeDriver: true }).start();
    };

    const fadeOrderOut = () => {
      Animated.timing(fadeOrderAnim.current, { toValue: 0, duration: 500, useNativeDriver: true }).start();
    };
    ...
    const fadeOutAll = () => {
      Animated.spring(translateValue.current, { toValue: { x: 0, y: 75 }, velocity: 4, tension: 10, friction: 2, useNativeDriver: true }).start();
    };
    ...
    const doEnyThings = (id, name, type) => {
      ...
      if (cloudSeaDiskStore.screenName === 'OnlineClassRoom') {
        console.log(id, name, type);
        if (typeof id === 'string' && typeof name === 'string' && typeof type === 'number' && type !== 0) {
          cloudSeaDiskStore.classSelectInfo = { resourceId: id, name, type };
          navigation.goBack();
        }
      } else {
        switch (type) {
          ...
          case 2:
            ...
            // 音频播放方案一 RNVoiceRecorder
            uploadFilesStore.getPublicFilePath(id).then((res) => {
              navigation.navigate('Main', { screen: 'AudioPlayer', params: { name: name, url: res, back: 'CloudSeaDisk' } });
            });
            break;
          case 3:
            uploadFilesStore.getPublicFilePath(id).then((res) => {
              navigation.navigate('Main', { screen: 'LessonVideoView', params: { name: name, url: res, back: 'CloudSeaDisk' } });
            });
            break;
          case 4:
          case 5:
    ...
    const selectPickerOpen = () => {
      ImagePicker.openPicker(paramsPicker)
        ...
        .catch(() => {
          baseView.current?.showMessage({ text: '取消图片选择', delay: 2 });
        });
    };
      ...
      switch (num) {
        ...
        case 8:
          if (select.length === 1 && cloudSeaDiskStore.allResourcesList.filter((item) => item.id === select[0]).length > 0) {
            ...
            baseView.current.showMessage({ text: '无法同时重命名多个文件', delay: '2' });
          }
          console.log(num, '准备修改文件名');
          break;
        case 9:
          if (select.length > 0) {
    ...
    const settingPublicFolder = (isPublic) => {
      ...
      if (cloudSeaDiskStore.allResourcesList.filter((item) => item.type === 0).length > 0) {
        baseView.current?.showMessage({ text: '文件夹无法被设置为共享', delay: 2 });
        return;
      }
      if (select.length < 2) {
            ...
            <TouchableHighlight
              ...
              onPress={() => {
                if (select.length > 0) {
                  ...
                  baseView.current?.showMessage({ text: '请选择文件夹', delay: 2 });
                }
              }}
              activeOpacity={6}
            >
    ...
    const menuMore = () => {
      return <BottomSheet ref={menuMoreSheet} snapPoints={[0, 250, 100]} borderRadius={20} renderContent={renderMenuSheet} />;
    };
    ...
    const loadAddData = () => {
      if (canAdd) {
        ...
        setTimeout(() => {
          cloudSeaDiskStore
            .addAccessToPublicResources({ current: cloudSeaDiskStore.allResourcesList.length, parentId: cloudSeaDiskStore.parentId, isAdd: true })
            .then(() => {
              canAdd = true;
            });
        }, 200);
    ...
    const editFolderName = () => {
      return (
        <Portal>
          ...
          >
            <View style={[tw.flex1, tw.mX0, tw.pY2, { height: 135, backgroundColor: colors.surface, borderTopLeftRadius: 12, borderTopRightRadius: 12 }]}>
    ...
    const renderContent = () => {
      return (
        <View style={[tw.pT5, tw.flex1]}>
          <View style={[tw.flexRow, tw.justifyBetween, tw.itemsCenter, tw.pB3, tw.pX2]}>
            ...
            {folderListAll.length > 0 ? (
              <TouchableOpacity
                onPress={() => {
                  setSelect([]);
                  if (cloudSeaDiskStore.allResourcesAllList.length > 2) {
          ...
          />
          {cloudSeaDiskStore.screenName === 'OnlineClassRoom' ? null : (
            <Animated.View
              style={[
                {
                  height: 75,
    ...
    return (
      <BaseView useSafeArea={false} scroll={false} ref={baseView} style={[tw.flex1]}>
        <Appbar.Header style={[tw.justifyBetween, tw.flexRow, tw.mX3, { backgroundColor: colors.background }]}>
          <View style={[tw.flexRow]}>
            ...
            >
              <View style={[tw.flexRow, tw.itemsCenter]}>
                <Text style={[tw.selfCenter, { fontSize: 18, fontWeight: 'bold', color: colors.text }]}>分类</Text>
          ...
          {select.length > 0 ? (
            <View style={[tw.flexRow, tw.borderGray200, tw.pY1, tw.pX2, { borderRadius: 20, borderWidth: 0.5 }]}>
              <View style={[tw.borderGray200, { borderRightWidth: 0.5 }]}>
                <Text
                  style={[{ padding: 3, fontSize: 16, color: colors.placeholder }]}
                  onPress={throttle(() => {
                    navigation.goBack();
                  }, 500)}
                >
                  {t('selectVodPage.cancel')}
                </Text>
              </View>
              <View>
                <Text
                  style={[{ padding: 3, fontSize: 16, color: colors.accent }]}
                  onPress={() => {
                    let list: any[] = [];
                    select.map((item) => {
                      list.push(cloudSeaDiskStore.allResourcesList.filter((items) => items.id === item)[0]);
                    });
                    let resList: ResourceFilesType[] = [];
                    list.map((item, index) => {
                      resList.push({
                        name: item.name,
                        resourceId: item.id,
                    ...
                    if (cloudSeaDiskStore.screenName === 'OnlineClassRoom') {
                      if (list.length > 1) {
                        baseView.current.showMessage({ text: '只能选择单个资源展示', delay: 2 });
                      } else {
                        cloudSeaDiskStore.classSelectInfo = resList[0];
                        navigation.goBack();
                      }
                    } else {
                      if (!createVodStore.jumpFromDraft) {
                        if (list.length > 0) {
                          if (!cloudSeaDiskStore.isEditVoidInfo) {
                            createVodStore.sectionVodInfo.push({
                              name: cloudSeaDiskStore.demandName,
                              resourceFiles: resList
                            });
                            navigation.goBack();
                          } else {
                            if (cloudSeaDiskStore.selectSessionIndex !== undefined) {
                              createVodStore.sectionVodInfo[cloudSeaDiskStore.selectSessionIndex].resourceFiles =
                                createVodStore.sectionVodInfo[cloudSeaDiskStore.selectSessionIndex].resourceFiles?.concat(resList) || [];
                        ...
                        if (list.length > 0) {
                          if (cloudSeaDiskStore.isEditVoidInfo) {
                            if (
                              (cloudSeaDiskStore.selectSessionIndex === 0 || cloudSeaDiskStore.selectSessionIndex) &&
                              createVodStore.lessonDetails?.schedules
                            ) {
                              createVodStore.lessonDetails.schedules[
                                cloudSeaDiskStore.selectSessionIndex
                              ].resourceFiles = createVodStore.lessonDetails?.schedules[cloudSeaDiskStore.selectSessionIndex].resourceFiles?.concat(resList);
                              createVodStore.lessonDetails.schedules = createVodStore.lessonDetails?.schedules.slice();
                              if (createVodStore.draftVodInfo) {
                                createVodStore.draftVodInfo = createVodStore.draftVodInfo.concat(resList);
                              }
                              navigation.goBack();
                            }
                          } else {
                            if (cloudSeaDiskStore.demandName) {
                              createVodStore.lessonDetails?.schedules?.push({
                                name: cloudSeaDiskStore.demandName,
                                resourceFiles: resList
                              });
                              navigation.goBack();
            ...
            >
              <View style={[tw.flexRow, tw.borderGray300, tw.pY1, tw.pX3, { borderRadius: 20, borderWidth: 0.5 }]}>
                <Text>返回</Text>
              </View>
            </TouchableOpacity>
          )}
        ...
        {renderContent()}
        {cloudSeaDiskStore.screenName === 'OnlineClassRoom' ? null : menuButton()}
        {menuMore()}
        {menuModal()}
        {createNewFile()}
        {deleteFolderModal()}
