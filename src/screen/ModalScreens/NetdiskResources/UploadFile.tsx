...
export const UploadFile: React.FC<NavigatorComponentProps & Props> = observer(
  ({ navigation }): JSX.Element => {
    ...
    const uploadPicker = useCallback(() => {
      console.log('走到这里', uploadFilesStore.temDocuments);
      uploadFilesStore.parentId = cloudSeaDiskStore.parentId;
      uploadFilesStore.getAsyncOssConfig().then((res) => {
        if (res) {
          console.log(res, '[log]');
          console.log(uploadFilesStore.aliOssConfigSetting, uploadFilesStore.uploadProgress, uploadFilesStore.temDocuments, 'trues');
    ...
    useEffect(() => {
      console.log(uploadFilesStore.temDocuments, '[log2]');
      if (uploadFilesStore.temDocuments.length > 0) {
        (async () => {
          await uploadPicker();
        })();
    ...
    const renderUploading = (data: any[], progress: number[]) => {
      return (
        <View>
